...
def process_line(slurmexe, options_one_line):
    ...
    try:
        ...
        print(f"Task with options {options_one_line} completed successfully.", flush=True)
...
def parser_args():
    ...
    parser.add_argument("--num-tasks", type=int, default=20, help="Number of tasks (unused if reading from file)", required=False)
    parser.add_argument("--timeout-min", type=int, default=20, help="Timeout in minutes for each task")
    parser.add_argument("--mem-gb", type=int, default=2, help="Memory in GB for each task")
    parser.add_argument("--cpus-per-task", type=int, default=1, help="Number of CPUs per task")
    parser.add_argument("--slurm-partition", type=str, default="cpu", help="Slurm partition to use")
    parser.add_argument('--listing-input', type=str, required=True, help="Path to a file containing input lines")
    parser.add_argument('--bash-slurm-exec', type=str, required=True, help="Path to the bash script")
    parser.add_argument("--output-dir", help="Directory to store submitit logs (suffixed with out/YYYYMMDDTHHMMSS/)", default="submitit_logs_array")
    parser.add_argument("--slurm-array-parallelism", type=int, default=20, help="Max concurrent tasks")
    return parser.parse_args()

def main(args):
    # Create log directory
    output_dir_with_date = os.path.join(args.output_dir,datetime.datetime.today().strftime('%Y%m%dT%H%M%S'))
    ...
    # Read inputs
    with open(args.listing_input, 'r') as f:
        # Filter out empty lines just in case
        array_inputs = [line.strip() for line in f.readlines() if line.strip()]
    ...
    print(f"Successfully submitted all {len(all_jobs)} tasks across multiple Job Arrays.")
    ...
    print("You can monitor specific logs using: tail -f {}/<JOB_ID>_<TASK_ID>_0.out".format(output_dir_with_date))
